...
- `IndirectControlMechanism`:
    ...
    - is has no direct `ControlFunction`, the function is obtained from the data member instead.
...
##### Description
`ControlParameter` is the storage of new objects generated by `generate(...)`.
...
- let `Function` returns the same result for those are stochastic on each call such as `RandomSelectionFunction`.
