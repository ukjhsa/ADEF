...
- they are dependent or independent to each individuals, and they are the same or different.
...
- Object type by template type
- `ControlRange` manages the range.
- `ControlParameter` manages objects, and `ControlFunction` manages `Function`.
-  `ControlSelection` adjusts by the relation between parent and offspring and `ControlUpdate` adjusts by the feature of current population.
