...
In DE, *F* and *CR* are the object and subjects represent
- they are `double`.
- they have the lower bound and upper bound to check whether they are valid.
- they are owned by all individuals or each individuals, and they are the same or different.
...
- they are adjusted on mutation and crossover.
- they are constant, random, generated by distribution, or by specific formula.

Therefore there are classes to support each subjects
- template type
- `ControlRange` manages the range.
- `ControlParameter` manages objects.
- `ControlMechanism::select(...)` use `ControlSelection` to adjust by the relation between parent and offspring and `ControlMechanism::update(...)` use `ControlUpdate` to adjust by the feature of current population.
- `generate(...)` on mutation and crossover.
- `Function` inside `ControlFunction` is the formula.
...
- `ControlFunction`: the storage of `Function`.
...
{
    // cr has ControlMechanism<double> type
