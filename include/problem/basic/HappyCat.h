...
HappyCat has extra configurations:
- member (optional)
    - name: "alpha"
    - value: @c double
.
...
/// @copydoc Problem::Object
    ...
    HappyCat() : Problem("HappyCat"), alpha_(0.0)
    {
    }
    HappyCat(const HappyCat& rhs) = default;

/**
@brief Clone the current class.
@sa clone_impl()
*/
    std::shared_ptr<HappyCat> clone() const
    {
        return std::dynamic_pointer_cast<HappyCat>(clone_impl());
    }
/**
@brief Set up the internal states.

If HappyCat has the following configuration:
- dimension_of_decision_space: 30
- lower_bound_of_decision_space: -100.0
- upper_bound_of_decision_space: 100.0
- optimal_solution: 0.0
...
*/
    void setup(const Configuration& config, const PrototypeManager& pm) override
    {
        ...
        alpha_ = alpha_config.is_null() ?
                                0.125 : alpha_config.get_value<Object>();
    }
    ...
    void evaluation_function(std::shared_ptr<Individual> individual) const override
    {
        ...
        Object result = std::pow(sum_square - dimension_of_decision_space_,
                                 2 * alpha_) +
...
private:
    ...
    {
        return std::make_shared<HappyCat>(*this);
    }
};
