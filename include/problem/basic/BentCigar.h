...
/**
@brief BentCigar function.

@par The configuration
BentCigar has no extra configurations.@n
...
*/
class BentCigar : public Problem
{
public:
...
/// @copydoc Problem::Object
    ...
    BentCigar() : Problem("Bent Cigar")
    {
    }
    BentCigar(const BentCigar& rhs) = default;

/**
@brief Clone the current class.
@sa clone_impl()
*/
    std::shared_ptr<BentCigar> clone() const
    {
        return std::dynamic_pointer_cast<BentCigar>(clone_impl());
    }
/**
@brief Set up the internal states.

If BentCigar has the following configuration:
- dimension_of_decision_space: 30
- lower_bound_of_decision_space: -100.0
- upper_bound_of_decision_space: 100.0
- optimal_solution: 0.0
...
*/
    ...
    {
        Object sum = individual->variables(0) * individual->variables(0);
        for (unsigned int idx = 1; idx < dimension_of_decision_space_; ++idx) {

            sum += 10.0 * 10.0 * 10.0 * 10.0 * 10.0 * 10.0 *
                   individual->variables(idx) * individual->variables(idx);
        }
...
private:
    ...
    {
        return std::make_shared<BentCigar>(*this);
    }
};
